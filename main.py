...
def start_training(character):
    """Переходим к функции запуска тренировки — start_training().
    В качестве параметра она должна принимать объект класса Character,
    который возвращает функция choice_char_class().
    Команды для тренировки можно вынести в словарь commands,
    где ключами будут строковые команды от пользователя,
    а значениями — методы объекта, который принимает функция.
    Блок, который отвечает за вывод описания персонажа, совсем уберите из кода проекта.

    Значением для ключа должен быть метод объекта, переданного в функцию вида объект_класса.метод.
    Скобки для метода указывать не нужно, потому что значением должен быть сам метод, а не его результат.
    Используйте оператор in для проверки принадлежности введённой команды словарю.
    В качестве аргумента для функции print() должен передаваться вызов метода объекта вида словарь[команда]()."""
